...
- **Atom growth (DoS).** `Session.open` names each agent with `String.to_atom("hns_agent_" <> unique)`. Atoms are never GC'd on the BEAM (~1M cap), and the atom leaks even after the session's agent is stopped, so lease-expiry does not bound it — sustained unauthenticated `GET /harness` traffic can exhaust the atom table. Deferred under the same "trusted network until auth" posture as the tool policy (R6.3): acceptable on a trusted network now; the fix (a bounded atom pool in the Registry, or upstream string-keyed agent registration in llmagent) lands with the auth/rate-limiting slice. Reviewed and accepted 2026-07-14.

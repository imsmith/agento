defmodule LlmagentWebWeb.ChatLive do
  @moduledoc """
  ...
  alias LlmagentWebWeb.Discovery.Agents
  ...
  def handle_event("send_prompt", %{"prompt" => text}, socket) do
    ...
    if text != "" and socket.assigns.selected_agent do
      ...
      LlmagentWebWeb.WebEvents.emit_prompt_sent(agent_name, text)
  ...
  def handle_event("start_agent", _params, socket) do
    ...
    case Agents.start(opts) do
      {:ok, _pid} ->
        LlmagentWebWeb.WebEvents.emit_agent_started(name, opts)
  ...
  def handle_event("stop_agent", %{"name" => name}, socket) do
    ...
    case Agents.stop(agent_name) do
      :ok ->
        LlmagentWebWeb.WebEvents.emit_agent_stopped(agent_name)
